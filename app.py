...
# Connect to MongoDB
client = MongoClient('mongodb://SMHV:SMHVisBEST2023!IsITreally%3F@10.0.0.4:27017/?directConnection=true&authMechanism=DEFAULT')
...
def show_organization(organization_id):
    ...
    demonstrations = list(protests.find({"organization._id": {"$in": [ObjectId(organization_id), organization_id]}}))
    return render_template('organization.html', organization=organization, demonstrations=demonstrations)
...
def demonstrations_by_topic(topic_id):
    ...
    return render_template('demonstrations_by_topic.html', topic=topic, demonstrations=demonstrations)
...
def manage_protests():
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            organization = organizations.find_one({'_id': ObjectId(request.form['new_organization'])})
...
def manage_organizations():
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            ...
            organizations.update_one({"_id": organization_id}, {"$set": {"name": updated_organization_name}})
        elif action == 'delete':
            organization_id = request.form['organization_id']
        ...
        if action == 'create' and password == confirm_password:
            ...
            if selected_user_id:
                ...
                if selected_user:
                    ...
                    users.update_one({"_id": ObjectId(selected_user_id)}, {"$set": selected_user})
        elif action == 'delete':
            selected_user_id = request.form.get('selected_user')
            if selected_user_id:
